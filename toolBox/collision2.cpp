...
bool intersectionPointPoint(Vector2f point_A, Vector2f point_B)
{
...
bool intersectionPointLine(Vector2f point, Line line)
{
    Vector2f vectAP = line.point-point;
    Vector2f normal_vect = (line.director.unit()).rotate90(); 
    return (abs(vectAP.dot(normal_vect)) < EPSILON);
}

//  Is there an intersection between this point and this segment?
bool intersectionPointSegment(Vector2f point, Segment seg)
{
    Line line(seg.a, (seg.b-seg.a).unit() );
...
bool intersectionPointCircle(Vector2f point, Circle cir)
{
    float magnitude_OP = (point-cir.o).magnitude();

    return (pow(magnitude_OP,2) <= pow(cir.radius,2));
}

//  Is there an intersection between this point and this box?
bool intersectionPointBox(Vector2f point, Box box)
{
...
bool intersectionLineLine(Line line_A, Line line_B)
{
...
bool intersectionLineSegment(Line line_A, Segment segment)
{
    Vector2f vectPA = line_A.point-segment.a;
    Vector2f vectPB = line_A.point-segment.b;

    Vector2f normal = line_A.director.unit().rotate90();
...
bool intersectionSegmentSegment(Segment segment_A, Segment segment_B)
{
    Vector2f dir_A = (segment_A.b-segment_A.a).unit();
    Vector2f dir_B = (segment_B.b-segment_B.a).unit();
...
bool intersectionSegmentCircle(Segment segment, Circle circle)
{
    Line line(segment.a,(segment.b-segment.a).unit() );
    if (!intersectionLineCircle(line,circle))
        return false;

    Vector2f OA = (segment.a-circle.o);
    if (OA.magnitude() < circle.radius)
        return true;

    Vector2f OB = segment.b-circle.o;
    if (OB.magnitude() < circle.radius)
        return true;
...
bool intersectionCircleCircle(Circle circle_A, Circle circle_B)
{
    return ((circle_B.o-circle_A.o).magnitude() <= (circle_A.radius + circle_B.radius));
}
...
bool intersectionCircleBox(Circle circle, Box box)
{
    if (intersectionPointBox(circle.o, box))
        return true;
    
    //nearest point of the box from the O() of circle
    Vector2f h(clampf(circle.o.x,box.o.x-box.width/2,box.o.x+box.width/2), clampf(circle.o.y, box.o.y-box.length/2, box.o.y+box.length/2));
    
    //vector circle O() to h
    Vector2f CH = circle.o-h;

    return circle.radius >= CH.magnitude(); 
}

//  Is there an intersection between two boxes?
bool intersectionBoxBox(Box box_A, Box box_B)
{
...
bool intersectionConvexPoint(ConvexPolygon convex, Vector2f point)
{   
    convex.makeSegments();
...
bool convexConvexTest(ConvexPolygon convex1, ConvexPolygon convex2)
{
    for (int i = 0; i < (int)convex1.segments.size(); i++)
    {
        Vector2f normal = convex1.segments[i].getNormal();
        Range2 current_range1 = range2Segment(convex1.segments[0], normal);
        for (int j = 1; j < (int)convex1.points.size(); j++)
            current_range1 = current_range1 + range2Segment(convex1.segments[j], normal);
      
        Range2 current_range2 = range2Segment(convex2.segments[0], normal);
...
bool intersectionConvexConvex(ConvexPolygon convex1, ConvexPolygon convex2)
{   
...
bool intersectionConvexCircle(ConvexPolygon convex, Circle circle)
{   
    convex.makeSegments();
    ...
    Vector2f normal = (nearest_point-circle.o).unit();
    Range2 current_range1 = range2Segment(convex.segments[0], normal);
    Range2 current_range2 = range2Circle(circle, normal);
